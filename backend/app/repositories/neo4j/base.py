...
class Neo4jRepositoryBase:
    ...
    def _run_single(self, *, query_name: str, query: str, **params: Any) -> Any:
        with self._driver.session(database=self._database) as session:
            return self._measure_query(query_name, lambda: session.run(query, **params).single())

    def _run_result(self, *, query_name: str, query: str, **params: Any) -> Any:
        with self._driver.session(database=self._database) as session:
            return self._measure_query(query_name, lambda: list(session.run(query, **params)))
